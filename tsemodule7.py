...
class StockManager:
    ...
    def stock(self, stockname="", value=100, newfile=False, standard=False, namadEn=False, progress=True):
        ...
        df['Adj Close'] = df['Close'] * (1 - 0.02)  # مثال: تعدیل قیمت به میزان 2%
    ...
    def get_highest_close_price_today(self, newfile=False, standard=False, progress=True):
        ...
        for stockname in self.stocknames:
            ...
            try:
                ...
                if progress:
                    print(f"Downloading new file for {stockname} ....")
                fopen = rq.get(url).content
                ...
                if "<DTYYYYMMDD>" in df.columns:
                    ...
                    print(f"Error: Column '<DTYYYYMMDD>' not found in {stockname} data")
                    continue

                df.to_csv(f"{stockname}.csv")

            if df.empty:
                # print(f"Skipping {stockname} due to empty DataFrame")
                continue
            ...
            if yesterday_data.empty:
                # print(f"No data for {stockname} on {yesterday_str}")
                continue
            ...
            if yesterday_data['Close'].dropna().empty:
                # print(f"No valid 'Close' data for {stockname} on {yesterday_str}")
                continue
